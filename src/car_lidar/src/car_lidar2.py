...
def Lidar_CB(msg):
    ...
    lidar_central_angle = 0
    scan_left_angle = 40 # degree
    scan_right_angle = -40 # degree
    range_per_resolution_angle =  1947 / 360 # range / degree 
    resolution_angle_radian_per_range = ( 2 * math.pi ) / 1947  # radian / range
    range_start = int(range_per_resolution_angle * scan_right_angle) 
    range_end = int(range_per_resolution_angle * scan_left_angle)
    
    require_max_distance = 3.0
    requirt_min_distance = 0.5

    print(msg)

    for i in range(range_start, range_end+1):
        # rospy.loginfo(f'{msg.ranges[i]}')
        if float(msg.ranges[i]) < require_max_distance and msg.ranges[i] > requirt_min_distance:
            rlt_dist = distance
            rlt_ang = angle
            # print("A")
            Get_GPS()
